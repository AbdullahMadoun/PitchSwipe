#!/usr/bin/env python3
"""
Revert companies back to Google sample videos (working state)
"""

import json
import os

# Google sample videos that were working
SAMPLE_VIDEOS = [
...
# Original Clearbit logos and UI Avatar images
COMPANY_LOGOS = {
...
def revert_companies():
    ...
    # Revert each company to sample videos
    companies = data.get("companies", {})
    video_index = 0
    
    for company_id, company in companies.items():
        # Revert to sample video URL (cycle through available videos)
        company["video_url"] = SAMPLE_VIDEOS[video_index % len(SAMPLE_VIDEOS)]
        video_index += 1
        
        # Revert to Clearbit logo if available
        if company_id in COMPANY_LOGOS:
            company["logo_url"] = COMPANY_LOGOS[company_id]
        
        # Revert to UI Avatar images if available
        if company_id in FOUNDER_IMAGES:
            company["founder_image"] = FOUNDER_IMAGES[company_id]
        
        # Keep the media flags
        company["video_activated"] = True
        company["has_media"] = True
        
        print(f"Reverted {company['name']} to sample video")
    
    # Save updated data
    with open(data_file, 'w') as f:
        json.dump(data, f, indent=2)
    
    print(f"\n✅ Successfully reverted {len(companies)} companies to sample videos!")
    print("Videos reverted to Google's sample video library")
    print("Logos reverted to Clearbit")
    print("Founder images reverted to UI Avatars")
